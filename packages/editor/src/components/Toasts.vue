...
<template>
  <Toaster :toaster="toaster" v-slot="toast">
    <Toast.Root :class="getToastRootStyles(toast.type || 'info')">
      <div
        class="group pointer-events-auto relative flex w-full min-w-96 max-w-lg justify-between gap-4 overflow-hidden rounded-lg border p-3 shadow-lg transition-all"
      >
        <!-- Content -->
        <div class="flex-1 min-w-0">
          ...
          <Toast.Description v-if="toast.description" class="mt-1 text-sm opacity-90">
            {{ toast.description }}
          </Toast.Description>
        ...
        <div class="flex items-start">
          <Toast.CloseTrigger
            class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md opacity-70 hover:opacity-100 transition-opacity"
          >
