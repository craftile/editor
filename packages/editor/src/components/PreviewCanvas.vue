...
const canvasStyle = computed(() => {
  ...
  const width = currentDeviceWidth.value === 0 ? '100%' : `${currentDeviceWidth.value}px`;

  // Disable transform for fit mode
  const transform = currentDeviceWidth.value === 0 ? 'none' : `scale(${scale})`;
...
<template>
  <div ref="previewWrapper" class="h-full w-full relative preview-wrapper overflow-hidden">
    <div
      ref="canvasContainer"
