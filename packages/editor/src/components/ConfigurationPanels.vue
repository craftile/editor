    ...
    <template v-if="hasSelection">
      <Tabs.Root v-if="showTabs" v-model="activePanelId" class="flex-1 flex flex-col overflow-y-hidden">
        ...
        <div class="flex-1 overflow-y-auto">
          <Tabs.Content v-for="panel in configurationPanels" :key="panel.id" :value="panel.id!">
            <component
      ...
      <div v-else-if="activePanel" class="flex-1 overflow-y-auto">
        <component
        ...
        <RenderFunctionWrapper v-else-if="isHtmlRenderFunction(activePanel.render)" :render-fn="activePanel.render" />
      </div>
    </template>
