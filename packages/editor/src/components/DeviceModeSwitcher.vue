<script setup lang="ts">
  ...
  const { t } = useI18n();
  const { currentDevice, currentDeviceData, devicePresets, savedCustomDevices, setDeviceMode, addCustomDevice, removeCustomDevice } = useDeviceMode();
      ...
      <Popover.Content class="w-72 border shadow-xs overflow-hidden mt-1 rounded-lg bg-white">
        <div class="flex items-center justify-between px-4 py-3 border-b border-solid">
          ...
          <Popover.CloseTrigger as-child>
            <button class="rounded cursor-pointer text-gray-500 transition-all duration-200 flex items-center justify-center p-1 hover:text-gray-700">
        ...
        <div class="flex flex-col gap-1 p-2">
          ...
          >
            <component
              :is="getIconComponent(device.icon)"
              class="w-5 h-5"
            />
