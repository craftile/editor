<script setup lang="ts">
  ...
  const { isEnabled, iframeRect, hoveredBlockId, hoveredBlockRect, hoveredParentRect, parentFlexDirection } = useInspector();
  ...
  const isVisible = computed(() => {
    return isEnabled.value &&
      ...
      hoveredBlockId.value !== selectedBlockId.value;
  });
...
<template>
  ...
  >
    ...
    >
      <!-- Preview label -->
      <div class="bg-blue-500 text-white px-1 py-0.5 rounded-t font-medium text-[0.60rem] flex items-center gap-0.5 pointer-events-none absolute -left-[1px] -top-[19px]">
        <span
